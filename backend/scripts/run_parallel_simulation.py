...
- Does not immediately close environment after simulation completes, enters command waiting mode
...
# This fixes OASIS third-party library issue with files not specifying encoding when read
# ============================================================
import sys
import os
...
if sys.platform == 'win32':
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None,
        ...
        """
        Wrapper for open() function, defaults to UTF-8 for text mode
        Fixes third-party libraries (e.g., OASIS) reading files without specified encoding
        """
...
class MaxTokensWarningFilter(logging.Filter):
    """Filter out camel-ai max_tokens warnings (we intentionally do not set max_tokens, letting model decide)"""
...
# Add filter immediately at module load, ensure it takes effect before camel code executes
...
def disable_oasis_logging():
    ...
    OASIS logs are too verbose (records each agent observation and action), we use our own action_logger
    """
...
# Non-core action types to filter out (these actions have lower analysis value)
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    ...
    This allows actions.jsonl to display real entity names instead of "Agent_0" placeholders
...
def fetch_new_actions_from_db(
    ...
    try:
        ...
        for rowid, user_id, action, info_json in cursor.fetchall():
            ...
            # Supplement with context information (post content, usernames, etc.)
...
def _enrich_action_context(
    ...
    """
    try:
        # Like/dislike post: supplement with post content and author
        ...
        # Repost: supplement with original post content and author
        ...
        # Quote post: supplement with original post content, author, and quote comment
        ...
        # Follow user: supplement with followed user's name
        ...
        # Mute user: supplement with muted user's name
        ...
        # Like/dislike comment: supplement with comment content and author
        ...
        # Create comment: supplement with commented post info
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    If boost LLM is configured, parallel simulation allows different platforms to use different API providers, improving concurrency.
...
async def main():
    ...
    # Create shutdown event at the beginning of main function, ensure the entire program responds to exit signals
...
def setup_signal_handlers(loop=None):
    ...
    When receiving a termination signal, it needs to:
