...
logger = get_logger('agenikpredict.api.simulation')


def _verify_project_access(project_id):
    """Verify current user has access to the project. Returns (project, error_response)."""
...
# Adding this prefix prevents Agent from calling tools, replying with text directly
...
def get_graph_entities(graph_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to get graph entities: {traceback.format_exc()}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
...
def get_entity_detail(graph_id: str, entity_uuid: str):
    ...
    try:
        ...
        logger.error(f"Failed to get entity details: {traceback.format_exc()}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
...
def get_entities_by_type(graph_id: str, entity_type: str):
    ...
    try:
        ...
        logger.error(f"Failed to get entities: {traceback.format_exc()}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
...
def create_simulation():
    ...
    Request (JSON):
        {
            ...
            "graph_id": "agenikpredict_xxxx",    // optional, if not provided, obtained from project
    ...
    try:
        ...
        logger.error(f"Failed to create simulation: {traceback.format_exc()}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    # Required files list (excludes scripts, scripts are in backend/scripts/)
    required_files = [
        "state.json",
...
def prepare_simulation():
    ...
    This is a time-consuming operation. Returns task_id immediately,
    ...
    Steps:
    1. Check for completed preparation work
    ...
    try:
        ...
        project, error = _verify_project_access(state.project_id)
        if error:
            return error
        ...
        def run_prepare():
            try:
                ...
                logger.error(f"Simulation preparation failed: {traceback.format_exc()}")
        ...
        logger.error(f"Failed to start preparation task: {traceback.format_exc()}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
...
def get_prepare_status():
    ...
    try:
        ...
        logger.error(f"Failed to query task status: {traceback.format_exc()}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
...
def get_simulation(simulation_id: str):
    """Get simulation status (ownership verified through parent project)"""
    try:
        manager = SimulationManager()
        state = manager.get_simulation(simulation_id)
        ...
        logger.error(f"Failed to get simulation status: {traceback.format_exc()}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
...
def list_simulations():
    ...
    try:
        ...
        owner_filter = None if getattr(g, 'user_role', '') == 'admin' else g.user_id
        simulations = manager.list_simulations(project_id=project_id, owner_id=owner_filter)
        ...
        logger.error(f"Failed to list simulations: {traceback.format_exc()}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
...
def get_simulation_history():
    ...
    For homepage historical project display, returns simulation list with project name, description, and other rich info
    ...
    try:
        ...
        for sim in simulations:
            ...
            # Get run status (read user-configured actual rounds from run_state.json)
            run_state = SimulationRunner.get_run_state(sim.simulation_id)
            if run_state:
                sim_dict["current_round"] = run_state.current_round
                sim_dict["runner_status"] = run_state.runner_status.value
                # Use user-configured total_rounds, fallback to recommended rounds if none
        ...
        logger.error(f"Failed to get simulation history: {traceback.format_exc()}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
...
def get_simulation_profiles(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to get profiles: {traceback.format_exc()}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
...
def get_simulation_profiles_realtime(simulation_id: str):
    """
    Get simulation agent profiles in real-time (used for real-time progress viewing during generation)

    Differences from /profiles endpoint:
    - Reads files directly, does not go through SimulationManager
    ...
    try:
        ...
        logger.error(f"Failed to get real-time profiles: {traceback.format_exc()}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
...
def get_simulation_config_realtime(simulation_id: str):
    """
    Get simulation config in real-time (used for real-time progress viewing during generation)
    ...
    try:
        ...
        logger.error(f"Failed to get real-time config: {traceback.format_exc()}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
...
def get_simulation_config(simulation_id: str):
    """
    Get simulation config (complete config from LLM intelligent generation)
    ...
    try:
        ...
        logger.error(f"Failed to get config: {traceback.format_exc()}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
...
def download_simulation_config(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to download config: {traceback.format_exc()}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
...
def download_simulation_script(script_name: str):
    ...
    try:
        ...
        logger.error(f"Failed to download script: {traceback.format_exc()}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
...
def generate_profiles():
    ...
    Request (JSON):
        {
            "graph_id": "agenikpredict_xxxx",     // required
    ...
    try:
        ...
        logger.error(f"Failed to generate profiles: {traceback.format_exc()}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
...
def start_simulation():
    ...
    About enable_graph_memory_update:
        ...
        - Requires simulation-associated project to have valid graph_id
    ...
    try:
        ...
        logger.error(f"Failed to start simulation: {traceback.format_exc()}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
...
def stop_simulation():
    ...
    try:
        ...
        logger.error(f"Failed to stop simulation: {traceback.format_exc()}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
...
def get_run_status(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to get run status: {traceback.format_exc()}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
...
def get_run_status_detail(simulation_id: str):
    ...
    try:
        ...
        # recent_actions Only show content from the latest round on both platforms
        ...
        logger.error(f"Failed to get detailed status: {traceback.format_exc()}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
...
def get_simulation_actions(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to get action history: {traceback.format_exc()}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
...
def get_simulation_timeline(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to get timeline: {traceback.format_exc()}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
...
def get_agent_stats(simulation_id: str):
    ...
    Used for frontend display of agent activity level rankings, action distribution, etc.
    """
    try:
        ...
        logger.error(f"Failed to get agent stats: {traceback.format_exc()}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
...
def get_simulation_posts(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to get posts: {traceback.format_exc()}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
...
def get_simulation_comments(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to get comments: {traceback.format_exc()}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
...
def interview_agent():
    ...
    Note: This feature requires the simulation environment to be in running status (enters command waiting mode after completing simulation loop)
    ...
    Request (JSON):
        {
            ...
            "platform": "twitter",             // optional, specify platform (twitter/reddit)
                                               // When not specified: dual platform simulation interviews on both platforms simultaneously
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "Simulation environment is not running or has been closed. Please ensure simulation is complete and in command waiting mode."
            }), 400
        ...
        logger.error(f"Interview failed: {traceback.format_exc()}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
...
def interview_agents_batch():
    ...
    Note: This feature requires the simulation environment to be in running status
    ...
    try:
        ...
        logger.error(f"Batch interview failed: {traceback.format_exc()}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
...
def interview_all_agents():
    ...
    try:
        ...
        logger.error(f"Global interview failed: {traceback.format_exc()}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
...
def get_interview_history():
    ...
    try:
        ...
        logger.error(f"Failed to get interview history: {traceback.format_exc()}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
...
def get_env_status():
    ...
    try:
        ...
        logger.error(f"Failed to get environment status: {traceback.format_exc()}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
...
def close_simulation_env():
    ...
    while this endpoint will gracefully close the simulation environment and exit.
    ...
    try:
        ...
        logger.error(f"Failed to close environment: {traceback.format_exc()}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
