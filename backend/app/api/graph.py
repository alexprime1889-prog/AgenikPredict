...
# Get logger
logger = get_logger('agenikpredict.api')
        ...
        if urls_raw.strip():
            ...
            for url in url_list:
                try:
                    ...
                    url_text = FileParser.extract_from_url(url)
                    url_text = TextProcessor.preprocess_text(url_text)
                    document_texts.append(url_text)
                    all_text += f"\n\n=== URL: {url} ===\n{url_text}"
                    project.files.append({"filename": url, "size": len(url_text)})
...
def build_graph():
    ...
    try:
        ...
        # Get configuration
        graph_name = data.get('graph_name', project.name or 'AgenikPredict Graph')
