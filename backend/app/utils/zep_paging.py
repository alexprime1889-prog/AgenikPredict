...
logger = get_logger('agenikpredict.zep_paging')
...
def _fetch_page_with_retry(
    ...
    **kwargs: Any,
    ...
    """Single page request with exponential backoff retry. Only retries network/IO transient errors."""
    if max_retries < 1:
...
def fetch_all_nodes(
    ...
    """Paginated graph node fetch, returns at most max_items (default 2000). Each page request has built-in retry."""
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """Paginated graph edge fetch, returns complete list. Each page request has built-in retry."""
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
