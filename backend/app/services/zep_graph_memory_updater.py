...
logger = get_logger('agenikpredict.zep_graph_memory_updater')
...
class AgentActivity:
    ...
    def to_episode_text(self) -> str:
        """
        Convert activity to text description sendable to Zep
    ...
    def _describe_create_comment(self) -> str:
        """Comment - includes comment content and commented post info"""
        content = self.action_args.get("content", "")
        post_content = self.action_args.get("post_content", "")
        post_author = self.action_args.get("post_author_name", "")
...
class ZepGraphMemoryUpdater:
    ...
    All meaningful behaviors will be updated to Zep. action_args will contain full context info:
    ...
    # Batch size (how many per platform before sending)
    BATCH_SIZE = 5
    ...
    # Send interval (seconds), avoid too rapid requests
    SEND_INTERVAL = 0.5
    ...
    def _flush_remaining(self):
        ...
        # First process remaining queue activities, add to buffers
        ...
        with self._buffer_lock:
            for platform, buffer in self._platform_buffers.items():
                if buffer:
                    display_name = self._get_platform_display_name(platform)
                    logger.info(f"Sending remaining {len(buffer)} activities from {display_name} platform")
