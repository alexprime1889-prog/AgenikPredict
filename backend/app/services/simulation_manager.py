...
logger = get_logger('agenikpredict.simulation')
...
class SimulationManager:
    ...
    def prepare_simulation(
        ...
        try:
            ...
            if filtered.filtered_count == 0:
                ...
                state.error = "No qualifying entities found, please check if graph is correctly built"
                self._save_simulation_state(state)
                return state
            ...
            # Save Profile files (note: Twitter uses CSV format, Reddit uses JSON)
    ...
    def list_simulations(self, project_id: Optional[str] = None, owner_id: Optional[str] = None) -> List[SimulationState]:
        """
        List simulations with optional filters.

        Args:
            project_id: Filter by project ID (optional)
            owner_id: Filter by owner user ID (optional). Legacy simulations (owner_id=None) are included.
        """
        simulations = []
    ...
    def get_run_instructions(self, simulation_id: str) -> Dict[str, str]:
        ...
        return {
            ...
            "instructions": (
                f"1. Activate conda environment: conda activate AgenikPredict\n"
