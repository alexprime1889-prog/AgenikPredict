...
logger = get_logger('agenikpredict.simulation_config')
...
class SimulationConfigGenerator:
    ...
    def __init__(
        ...
        # Default to no language instruction (English)
    ...
    def generate_config(
        ...
        # Store language instruction for LLM prompts
        self._language_instruction = get_llm_language_instruction(language)

        # Calculate total steps
        num_batches = math.ceil(len(entities) / self.AGENTS_PER_BATCH)
        total_steps = 3 + num_batches  # Time config + event config + N batches Agent + platform config
        current_step = 0
        ...
        # ========== Steps 3-N: Generate Agent config in batches ==========
        all_agent_configs = []
        ...
        # ========== Assign publisher Agent to initial posts ==========
        logger.info("Assigning suitable publisher Agent to initial posts...")
...
- 9 AM-6 PM: moderately active work hours (activity multiplier 0.7)
- 7-10 PM: peak hours (activity multiplier 1.5)
- After 11 PM: decreasing activity (activity multiplier 0.5)
- General pattern: low early morning, increasing morning, moderate work hours, evening peak
- **Important**: Example values below are for reference only, adjust based on event nature and participant group characteristics
  - E.g., student peak may be 9-11 PM; media active all day; official agencies only during work hours
...
- reasoning (string): Brief explanation of why this configuration"""
        ...
        system_prompt = "You are a social media simulation expert. Return pure JSON format, time config should follow Chinese daily schedule." + self._language_instruction
...
{{
    ...
    "reasoning": "<brief explanation>"
        ...
        system_prompt = "You are a public opinion analysis expert. Return pure JSON format. Note poster_type must exactly match available entity types." + self._language_instruction
    ...
    def _generate_agent_configs_batch(
        ...
        prompt = f"""Generate social media activity configuration for each entity based on the following info.
...
- **Time follows Chinese daily schedule**: almost no activity 0-5 AM, most active 7-10 PM
...
{{
    "agent_configs": [
        ...
        system_prompt = "You are a social media behavior analysis expert. Return pure JSON, config should follow Chinese daily schedule." + self._language_instruction
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        """Rule-based generation for single Agent config (Chinese daily schedule)"""
