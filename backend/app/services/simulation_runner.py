...
logger = get_logger('agenikpredict.simulation_runner')
...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        try:
            ...
            # This fixes third-party libraries (e.g. OASIS) issue with files not specifying encoding when read
            env = os.environ.copy()
                        ...
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                # Update round info (from round_end event)
                                    ...
                                    # Update each platform's independent rounds and time
                                    if platform == "twitter":
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        Check whether all enabled platforms have completed by checking if corresponding actions.jsonl files exist
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        ...
        """
        if IS_WINDOWS:
            # Windows: use taskkill command to kill process tree
    ...
    def cleanup_all_simulations(cls):
        ...
        Called on server shutdown, ensures all child processes are terminated
        """
        ...
        # Check if there is anything to clean up (avoid idle processes printing useless logs)
    ...
    def register_cleanup(cls):
        ...
        Called on Flask app startup, ensures all simulation processes are cleaned up on server shutdown
        """
        ...
        # In Flask debug mode, only register cleanup in reloader subprocess (the actual running app process)
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Specified platform (optional)
                ...
                - None: For dual platform simulations, interview on both platforms, return integrated results
    ...
    def interview_all_agents(
        ...
        Use the same questions to interview all simulationAgent

        Args:
            simulation_id: Simulation ID
            prompt: Interview questions (same question for all Agents)
            platform: Specified platform (optional)
                - "twitter": Interview on Twitter platform only
                - "reddit": Interview on Reddit platform only
                - None: For dual platform simulations, interview each Agent on both platforms
        ...
        # Get all Agent info from config files
        config_path = os.path.join(sim_dir, "simulation_config.json")
        if not os.path.exists(config_path):
    ...
    def close_simulation_env(
        ...
        """
        Shut down simulation environment (not stop simulation process)
        
        Send shutdown environment command to simulation for graceful exit from command-waiting mode
        ...
        logger.info(f"Sending shutdown environment command: simulation_id={simulation_id}")
