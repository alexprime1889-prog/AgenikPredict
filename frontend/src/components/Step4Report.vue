...
const toolConfig = {
  ...
  'panorama_search': {
    ...
    color: 'blue',
    icon: 'globe' // Panorama search icon
  },
  'interview_agents': {
    name: 'Agent Interview',
    color: 'green',
    icon: 'users' // Interview icon
  },
  'quick_search': {
    name: 'Quick Search',
    color: 'orange',
    icon: 'zap' // Quick search icon
  },
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Extract core entities - with summary and related fact count
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        if (!twitterMatch && redditMatch) {
          ...
          // No platform markers (very old format), use as whole answer
          interview.twitterAnswer = answerText
        }
      }
      
      // Extract key quotes (compat with various quote formats)
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. "Question N:" format
      // 2. Number+dot format (legacy compat)
      let matches = []
      let match

      // Prefer "Question N:" format first
      const cnPattern = /(?:^|[\r\n]+)Question (\d+)[：:]\s*/g
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual-platform toggle (only show when both have real answers)
...
const renderMarkdown = (content) => {
  ...
  // Remove leading h2 (## xxx) since section title shown externally
  ...
  // Handle breaks - blank lines become paragraphs, single newline becomes <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // Fix ordered list numbering when single-item <ol> are separated by paragraphs
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (newLogs.length > 0) {
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // If task complete, scroll to top; otherwise scroll to bottom for latest logs
            if (isComplete.value) {
...
const extractFinalContent = (response) => {
  ...
  // Try to find content after Final Answer: (various formats)
  ...
  // If content is long with markdown, try removing thinking process
