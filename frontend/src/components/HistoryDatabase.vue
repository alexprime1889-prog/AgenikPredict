<template>
  ...
  >
    <!-- Background decoration: tech grid lines (only show when projects exist) -->
    ...
    <!-- Cards container (only show when projects exist) -->
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              <div class="modal-section">
                <div class="modal-label">{{ $t('history.simulationReq') }}</div>
            ...
            <div class="modal-divider">
              ...
              <span class="divider-text">{{ $t('history.playbackTitle') }}</span>
              <span class="divider-line"></span>
            </div>
...
let pendingState = null  // Target state to execute
...
const containerStyle = computed(() => {
  ...
  // Calculate required height: rows * card height + (rows-1) * gap + bottom padding
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // If target matches current state, no action needed
        if (shouldExpand === isExpanded.value) {
          pendingState = null
          return
        }
        
        // Debounce state switch to prevent rapid flicker
    ...
    },
    {
      ...
      // Adjust rootMargin to require more scrolling before expanding
...
.grid-pattern {
  ...
  /* Position from top-left, expand only at bottom on height change */
