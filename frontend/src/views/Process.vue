<template>
  <div class="process-page">
    <!-- Top Navigation -->
    <nav class="navbar">
      <div class="nav-brand" @click="goHome">{{ $t('brand') }}</div>
    ...
    <div class="main-content">
      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        <div class="panel-header">
          <div class="header-left">
            ...
            <span class="header-title">{{ $t('graph.title') }}</span>
          </div>
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-else-if="graphLoading" class="graph-loading">
            ...
            <p class="loading-text">{{ $t('graph.loading') }}</p>
          </div>
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-text">{{ $t('graph.waitingData') }}</p>
            <p class="waiting-hint">{{ $t('graph.waitingDesc') }}</p>
          </div>
          ...
          <div v-else-if="currentPhase === 1 && !graphData" class="graph-waiting">
            ...
            <p class="waiting-text">{{ $t('steps.graphBuild') }}</p>
            <p class="waiting-hint">{{ $t('graph.loading') }}</p>
          </div>
      ...
      <div class="right-panel" :class="{ 'hidden': isFullScreen }">
        <div class="panel-header dark-header">
          ...
          <span class="header-title">{{ $t('step1.title') }}</span>
        </div>
        ...
        <div class="process-content">
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 0, 'completed': currentPhase > 0 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                <div class="phase-title">{{ $t('step1.phase.ontology') }}</div>
            ...
            <div class="phase-detail">
              ...
              <div class="detail-section waiting-state" v-if="!projectData?.ontology && currentPhase === 0 && !ontologyProgress">
                <div class="waiting-hint">{{ $t('step1.noLogs') }}</div>
              </div>
            </div>
          </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">Description</div>
                <div class="detail-content">
                  Based on the generated ontology, documents are chunked and sent to Zep API to build a knowledge graph, extracting entities and relationships
                </div>
              </div>
              ...
              <div class="detail-section" v-if="graphData">
                ...
                <div class="build-result">
                  <div class="result-item">
                    ...
                    <span class="result-label">{{ $t('graph.nodes') }}</span>
                  </div>
                  <div class="result-item">
                    <span class="result-value">{{ graphData.edge_count }}</span>
                    <span class="result-label">{{ $t('graph.edges') }}</span>
                  </div>
                  <div class="result-item">
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 2, 'completed': currentPhase > 2 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                <div class="phase-title">{{ $t('step1.phase.complete') }}</div>
                <div class="phase-api">{{ $t('step1.nextStep') }}</div>
              </div>
...
const handleNewProject = async () => {
  ...
  if (!pending.isPending || pending.files.length === 0) {
    error.value = 'No pending files found. Please return to home page and try again.'
    loading.value = false
    return
  }
  ...
  try {
    ...
    if (pending.enrichWithMarketData) {
      formDataObj.append('enrich_with_market_data', 'true')
    }
...
.process-page {
  min-height: 100vh;
  background: #000000;
  font-family: 'JetBrains Mono', 'Noto Sans Hebrew', monospace;
  overflow: hidden;
  color: #E0E0E0;
}
